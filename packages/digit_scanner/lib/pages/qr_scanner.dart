...
// ---------- derived config (validations take precedence) ----------
  ...
  void initState() {
    _applicationIdentifier = () {
      ...
      if (widget.validations != null) {
        for (final v in widget.validations!) {
          try {
            if (v.type == 'applicationIdentifiers') {
              raw = v.value;
              break;
            }
          } catch (_) {}
    ...
    _mlKitScanner = mlkit.BarcodeScanner(formats: [
    ...
    // Start ML Kit periodic scanning (runs alongside ZXing)
    _mlKitTimer = Timer.periodic(const Duration(seconds: 3), (_) {
      _processFrameWithMLKit();
    });
  ...
  Future<void> _processFrameWithMLKit() async {
    if (_isProcessing || manualCode) return;

    try {
      final String? filePath = await _captureChannel.invokeMethod('capture');
      if (filePath == null || !mounted) return;

      debugPrint('[DigitScanner] ML Kit processing captured frame: $filePath');

      final inputImage = mlkit.InputImage.fromFilePath(filePath);
      final barcodes = await _mlKitScanner.processImage(inputImage);

      debugPrint('[DigitScanner] ML Kit found ${barcodes.length} barcodes');
      ...
      debugPrint(
          '[DigitScanner] ML Kit detected: format=${barcode.format} code=${barcode.rawValue}');

      setState(() => _isProcessing = true);

      await DigitScannerUtils().processScannedCode(
        context: context,
      ...
      debugPrint('[DigitScanner] ML Kit error: $e');
    }
  }
  ...
  scanWidget(BuildContext context, ThemeData theme, DigitTextTheme textTheme,
    ...
    return Stack(
      children: <Widget>[
        ...
        Positioned(
          top: MediaQuery.of(context).padding.top + spacer1,
          left: spacer1,
          child: IconButton(
            icon: Icon(
              Icons.arrow_back,
